...
序号|步骤|过程|备注
---|---|---|---
1|计算"应得伤害"（非神圣伤害）|ATK/M_ATK 与 DEF/M_DEF 参与计算,DEF对ATK的伤害进行扣除| <ul><li>类似造成120%ATK伤害，这样的技能，是指参与计算的ATK *120%. </li> <li>类似"无视40%"物理/魔法防御这样的技能，是指参与计算的DEF/M_DEF 被扣除</li></ul>
2|计算"应得伤害"（神圣伤害）|ATK/M_ATK参与计算,DEF对ATK无影响（无视防御)|类似造成120%ATK的神圣伤害，这样的技能，"应得伤害"就是120% * ATK
3|计算"实际伤害"|"应得伤害"为输入，考虑"暴击"、"减伤"因素|暴击情况下，"实际伤害"="应得伤害"X2（暴击基础倍率）X 暴击伤害加成. 而减伤，在最后一个环节进行减少即可

> 注意，减少伤害因为是在最后一步计算，所以收益非常大。和扣除对方的ATK/M_ATK是完全不同的计算方式
...
判断战斗结束的条件，是一个队伍中的所有英雄"完全死亡"(`completeDead`),"完全死亡"必须满足以下条件：

- HP <= 0
- 英雄身上没有`reborn`效果（重生效果，会标记在英雄身上，在特定生命周期，复活英雄）

在战斗中，一个英雄一旦`HP<0`,就会触发"dead"事件，但是并不代表他`完全死亡`。

> 所以，一些技能比如"队友死亡给自己增加20%ATK",当你的队友有重生技能的时候，会更加强大（多次触发）