...
describe("poison :", function () {
    ...
    it("poison should work,with deadly=false, target could not die", function () {
        ...
        expect(poisonEffect.toString()).to.eql("中毒效果(不致死)[持续2/2回合],每回合造成基于50% M_ATK的伤害,无视%100防御[不可被清除]");
        //身上有效果
        ...
        expect(poisonEffect.toString()).to.eql("中毒效果(不致死)[持续1/2回合],每回合造成基于50% M_ATK的伤害,无视%100防御[不可被清除]");
        //身上有效果
        expect(target.effects.length).to.eql(1);
        expect(target.effects[0].name).to.eql("Poison");
        //英雄的HP 减少
        ...
        //模拟经过2个回合
        worldContext.emit(WordLifeCycle.TURN_END);
        expect(poisonEffect.toString()).to.eql("中毒效果(不致死)[持续0/2回合],每回合造成基于50% M_ATK的伤害,无视%100防御[不可被清除]");
        //英雄的HP 减少
    ...
    it("poison should work,with deadly=true, target could die", function () {
        ...
        expect(poisonEffect.toString()).to.eql("中毒效果(致死)[持续2/2回合],每回合造成基于150% M_ATK的伤害[不可被清除]");
        //身上有效果
        expect(target.effects.length).to.eql(1);
        expect(target.effects[0].name).to.eql("Poison");
        //放置效果后，英雄的HP 不变
        ...
        //模拟经过1个回合
        worldContext.emit(WordLifeCycle.TURN_END);
        expect(poisonEffect.toString()).to.eql("中毒效果(致死)[持续1/2回合],每回合造成基于150% M_ATK的伤害[不可被清除]");
        //身上有效果
        expect(target.effects.length).to.eql(1);
        expect(target.effects[0].name).to.eql("Poison");
        //英雄的HP 减少
        ...
        //模拟经过2个回合
        worldContext.emit(WordLifeCycle.TURN_END);
        expect(poisonEffect.toString()).to.eql("中毒效果(致死)[持续0/2回合],每回合造成基于150% M_ATK的伤害[不可被清除]");
        //英雄的HP 减少
